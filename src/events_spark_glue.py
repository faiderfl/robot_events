...
def get_orb_site_machines(spark) -> DataFrame:
  ...
  orb_site_info = spark.read.format("csv").option("header", "true").option("sep",",").load("s3://robots-events/orb_site_info.csv")
  orb_site_info = orb_site_info.withColumnRenamed('orb_site','meta_orb_site')
  machines = spark.read.format("csv").option("header", "true").option("sep",",").load("s3://robots-events/orb_site_machines.csv")
...
def get_sorting_states(spark) -> DataFrame:
  ...
  sorting_states = spark.read.format("csv").option("header", "true").option("sep",",").load("s3://robots-events/sorting_states.csv")
...
def get_sort_attempts(spark) -> DataFrame:
  ...
  sort_attempts = spark.read.format("csv").option("header", "true").option("sep",",").load("s3://robots-events/sort_attempts.csv")
...
def get_unloads(spark) -> DataFrame:
  ...
  unloads = spark.read.format("csv").option("header", "true").option("sep",",").load("s3://robots-events/unloads.csv")
  unloads_items = spark.read.format("csv").option("header", "true").option("sep",",").load("s3://robots-events/unload_items.csv")
...
hourly_sorting_aggregate.write.format("jdbc").\
    option("url", "jdbc:redshift://redshift-cluster-robots.cl9b0baloazv.us-east-1.redshift.amazonaws.com:5439/dev").\
    option("dbtable", "public.hourly_sorting_aggregate").\
    option("user", "").\
    option("password", "").\
    mode('overwrite').save()
